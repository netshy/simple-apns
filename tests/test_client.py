...
from simple_apns.exceptions import APNSException, APNSTokenError, APNSServerError
...
def test_client_init_with_custom_params(apns_test_params):
    ...
    custom_params.update({
        "apns_topic": "com.example.customtopic",
        "timeout": 15,
        "max_retries": 5
    })
...
def test_get_auth_token(mock_client):
    ...
    with patch('simple_apns.client.create_token', return_value="mock_token") as mock_create_token:
...
def test_get_headers(mock_client):
    ...
    with patch.object(mock_client, '_get_auth_token', return_value="test_token"):
        ...
        headers = mock_client._get_headers("device_token", expiration=1600000000, priority=5)

        assert headers["apns-expiration"] == "1600000000"
        assert headers["apns-priority"] == "5"


def test_send_notification_success(mock_client, sample_payload, sample_device_token, mock_response_success):
    ...
    with patch.object(mock_client, '_get_headers', return_value=test_headers):
        with patch.object(mock_client.client, 'post', return_value=mock_response_success) as mock_post:
            # Call the method
            ...
            assert call_args[0][0] == f"{mock_client.endpoint}/3/device/{sample_device_token}"
            assert call_args[1]["json"] == sample_payload.to_dict()
            assert call_args[1]["headers"] == test_headers
            assert call_args[1]["timeout"] == 10


def test_send_notification_with_dict_payload(mock_client, sample_device_token, mock_response_success):
    ...
    with patch.object(mock_client, '_get_headers', return_value=test_headers):
        with patch.object(mock_client.client, 'post', return_value=mock_response_success) as mock_post:
            ...
            dict_payload = {"aps": {"alert": {"title": "Dict Test", "body": "Test body"}}}
...
def test_send_notification_bad_token(mock_client, sample_payload, sample_device_token, mock_response_error):
    """Test sending a notification with a bad device token."""
    with patch.object(mock_client, '_get_headers', return_value={"some": "headers"}):
        with patch.object(mock_client.client, 'post', return_value=mock_response_error) as mock_post:
...
def test_send_notification_server_error_with_retry(mock_client, sample_payload, sample_device_token):
    ...
    with patch.object(mock_client, '_get_headers', return_value={"some": "headers"}):
        ...
        with patch.object(mock_client.client, 'post', side_effect=[server_error, success_response]) as mock_post:
...
def test_send_notification_max_retries_exceeded(mock_client, sample_payload, sample_device_token):
    ...
    with patch.object(mock_client, '_get_headers', return_value=test_headers):
        ...
        with patch.object(mock_client.client, 'post', return_value=server_error) as mock_post:
            with patch('time.sleep') as mock_sleep:  # Mock sleep to speed up test
                ...
                # Check call counts
                assert mock_post.call_count == 1 + mock_client.max_retries  # Initial + retries
                assert mock_sleep.call_count == mock_client.max_retries  # Sleep between retries


def test_send_notification_network_error(mock_client, sample_payload, sample_device_token):
    """Test sending a notification with a network error."""
    with patch.object(mock_client, '_get_headers', return_value={"some": "headers"}):
        # Mock post to raise a network error
        with patch.object(mock_client.client, 'post', side_effect=httpx.RequestError("Network error")) as mock_post:
            with patch('time.sleep') as mock_sleep:  # Mock sleep to speed up test
...
def test_send_notification_with_custom_options(mock_client, sample_payload, sample_device_token, mock_response_success):
...
def test_send_bulk_notifications(mock_client, sample_payload):
    ...
    with patch.object(mock_client, 'send_notification', side_effect=mock_send) as mock_send_notification:
        ...
        mock_send_notification.assert_has_calls([
            call(device_token="token1", payload=sample_payload, push_type="alert", priority=10, expiration=None,
                 collapse_id=None),
            call(device_token="token2", payload=sample_payload, push_type="alert", priority=10, expiration=None,
                 collapse_id=None),
            call(device_token="token3", payload=sample_payload, push_type="alert", priority=10, expiration=None,
                 collapse_id=None)
        ], any_order=False)
...
def test_send_bulk_notifications_with_options(mock_client, sample_payload):
    ...
    with patch.object(mock_client, 'send_notification', return_value=True) as mock_send_notification:
        results = mock_client.send_bulk_notifications(
            device_tokens=device_tokens,
            payload=sample_payload,
            push_type="background",
